...
var debug = require('debug')('graphdb-orient');
var orientDebug = require('debug')('orientdb');
...
var templates = {
  selectById: 'SELECT FROM <%= type %> WHERE _id = "<%= id %>"',
  selectEdge: 'SELECT FROM <%= type %> ' +
  'WHERE in._id = "<%= source.id %>" ' +
  'AND out._id = "<%= target.id %>"',

  update: 'UPDATE <%= type %> <%= sqlsets %> WHERE _id = "<%= id %>"',
  vertexCreate: 'CREATE VERTEX <%= type %> <%= sqlsets %>',
  edgeCreate: 'CREATE EDGE <%= type %> FROM ' +
  '(SELECT FROM <%= source.type %> WHERE _id = "<%= source.id %>") TO ' +
  '(SELECT FROM <%= target.type %> WHERE _id = "<%= target.id %>") ' +
  '<%= sqlsets %>'
};
...
exports.activateNodeType = function(graph, definition, callback) {
  debug('activating node type: ', definition);
  activateType(graph, definition, DEFAULT_TYPE_VERTEX, callback);
};
...
exports.activateEdgeType = function(graph, definition, callback) {
  debug('activating edge type: ', definition);
  activateType(graph, definition, DEFAULT_TYPE_EDGE, callback);
};

exports.remapId = function(data) {
  return data._id;
};
...
exports.find = function(graph, searchParams, opts, callback) {
  ...
  // and then a graph edge if no vertex is found
    ...
    findById(graph, searchParams.id, DEFAULT_TYPE_VERTEX, function(err, results) {
      ...
      return findById(graph, searchParams.id, DEFAULT_TYPE_EDGE, callback);
    });
  }
    else {
...
exports.getEdge = function(graph, source, target, edgeType, callback) {
  var command = templates.selectEdge({
    type: edgeType || DEFAULT_TYPE_EDGE,
    source: source,
    target: target
  });
  ...
  debug('looking for edge: ', command);
...
exports.saveNode = function(graph, node, callback) {
  ...
  node.type = node.type || DEFAULT_TYPE_VERTEX;
...
exports.saveEdge = function(graph, source, target, entity, callback) {
  ...
  exports.getEdge(graph, source, target, entity.type, function(err, existing) {
    var commandTemplate = templates[existing ? 'update' : 'edgeCreate'],
        ...
        commandText = commandTemplate({
          type: entity.type || DEFAULT_TYPE_EDGE,
          sqlsets: orienteer.objectTo('SET', data),
          source: source,
          target: target,
    ...
    debug('saving edge, type: ' + (entity.type || DEFAULT_TYPE_EDGE), commandText);
